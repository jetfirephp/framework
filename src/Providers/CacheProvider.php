...
class CacheProvider extends Provider{
    ...
    public function __construct($config = [],$env){
        $this->config = $config;
        $this->cache = call_user_func_array([$this,$this->cacheDrivers[$config['drivers'][$config[$env]]['use']]],[$config['drivers'][$config[$env]]]);
        $this->addAlias($config[$env],$config['drivers'][$config[$env]]['use']);
    }

    public function getCache($key = null){
        if(is_null($key))
            return $this->cache;
        return call_user_func_array([$this,$this->cacheDrivers[$this->config['drivers'][$key]['use']]],[$this->config['drivers'][$key]]);
    }
    ...
    private function getFileCache($driver){
        ...
        $this->register($driver['use'],[
            'shared' => true,
            'construct' => [$driver['args'][0],$driver['args'][1]]
        ]);
        return $this->get($driver['use']);
    }
